...
const JWT_SECRET = 'kaimanga_secret_key_123'; // In production, use environment variable
...
router.post('/auth/register', async (req, res) => {
  ...
  try {
    ...
    res.json({ user: { id: result.insertId, username }, token });
  } catch (err) {
...
router.get('/bookmarks', authenticateToken, async (req, res) => {
  try {
    const [bookmarks] = await pool.execute(
      'SELECT manga_id as id, title, image, latest_chapter_name, latest_chapter_id FROM bookmarks WHERE user_id = ? ORDER BY created_at DESC',
      [req.user.id]
    );
    
    // Format to match frontend Manga type
    const formatted = bookmarks.map(b => ({
      id: b.id,
      title: b.title,
      image: b.image,
      latestChapter: b.latest_chapter_id ? { id: b.latest_chapter_id, name: b.latest_chapter_name } : undefined
    }));
...
router.post('/bookmarks', authenticateToken, async (req, res) => {
  ...
  if (!manga || !manga.id) return res.status(400).json({ error: 'Invalid manga data' });

  try {
    await pool.execute(
      'INSERT INTO bookmarks (user_id, manga_id, title, image, latest_chapter_name, latest_chapter_id) VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE title = VALUES(title), image = VALUES(image), latest_chapter_name = VALUES(latest_chapter_name), latest_chapter_id = VALUES(latest_chapter_id)',
      [
        req.user.id, 
        manga.id, 
        manga.title, 
        manga.image, 
        manga.latestChapter?.name || null, 
        manga.latestChapter?.id || null
      ]
    );
...
router.delete('/bookmarks/:mangaId', authenticateToken, async (req, res) => {
  try {
    await pool.execute(
